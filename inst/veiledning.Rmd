...
### Test of staging data
Please remove this section once no longer needed for testing of staged data.

```{r initStagingData, echo=FALSE, eval=TRUE}
testData <- data.frame()
```

```{r stagingDataTest, echo=FALSE, eval=FALSE}
...
if (isFALSE(testData)) {
  message("Staged data not found. I will create it!")
  testData <- dplyr::storms
  rapbase::saveStagingData(
    registryName = "ablanor",
    dataName = "ablanorData",
    data = testData
  )
} else {
  message("Staging data found. Cool!")
}
```

```{r stagingDataStatus, echo=FALSE, eval=TRUE}
if (dim(testData)[1] == 0) {
  cat("Data set is empty. Most likely, the test is not being evaluated.\n")
}
```

Staging data consists of `r dim(testData)[1]` observations on `r dim(testData)[2]` variables.
