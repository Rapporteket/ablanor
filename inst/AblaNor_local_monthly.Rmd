...
proms_1aar_antPros_fjor <- d_ablanor_prom1aar %>% 
  filter(centreid %in% params$reshId,
         aar_prosedyre %in% periode_data$sisteHeleYear,
         !is.na(.data$forlopstype)) %>% 
  nrow()
...
responsrate_1aar_lokalt_fjor <- data.frame(
  ...
  ant_pros = proms_antPros_toAarSiden,
  datagrunnlag = proms_datagrunnlag_toAarSiden,
  besvart = proms_besvart_toAarSiden) %>% 
...
proms_1aar_antPros_nasj_fjor <- d_ablanor_prom1aar %>% 
  filter(aar_prosedyre %in% periode_data$sisteHeleYear,
         !is.na(.data$forlopstype)) %>% 
  nrow()
...
if(params$tableFormat %in% "html") {
  responsrate_1aar_fjor %>% rename(" " = "sykehus") %>%   
  rapbase::mst(.,
               type = params$tableFormat,
               cap = cap,
               digs = 1,
