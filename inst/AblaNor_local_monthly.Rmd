...
caption <- paste0(
  "Tabellen viser responsrate for prosedyrene som ble utført i perioden ",
  (periode_data$forste_dato_1aarOppf), " og " ,
  (periode_data$siste_dato_1aarOppf),
...
```{r figKliniskEffekt1aar, fig.cap = fig.cap, fig.pos = "H", fig.align = 'center',  out.width = "80%", fig.asp = 0.8}

fig.cap <-  paste0("Figurene under viser pasientraportert klinisk effekt 1 år ",
                   " etter prosedyren utført i perioden ", 
                   (periode_data$forste_dato_1aarOppf), " og " ,
                   (periode_data$siste_dato_1aarOppf), ", ",
                   "i prosent for ",
                   hospitalNameShort, " og nasjonalt der oppfølgingen ble ",
                   "besvart i ", periode_data$sisteHeleYear,
                   " samt alle fullførte måneder hittil i ",
                   periode_data$nyesteRegYear, ".")
...
klinisk_effekt_1ar <- rbind(
  ...
  mutate(aar_prosedyre = factor(
    ...
    labels = c(paste0("Totalt for prosedyrer utført i ", (periode_data$sisteHeleYear-1)),
               paste0("Hittil i år for prosedyrer utført i ", (periode_data$nyesteRegYear-1)))
    ))
...
# klinisk_effekt_1ar %>% 
  ggplot2::ggplot(
    mapping = aes(fill = followup1_q2_tekst,
                  y = n,
                  x = factor(sykehusnavn))) +
  ggplot2::geom_bar(position = "fill",
                    stat = "identity",
                    width = 0.6) +
  ggplot2::guides(fill = guide_legend(reverse = TRUE)) +
  ggplot2::scale_fill_manual(values = colPrimary) +
  ggplot2::facet_wrap(~ aar_prosedyre, ncol = 5, strip.position = "top") +
  xlab("") +
  # ggplot2::ggtitle("Klinisk effekt", subtitle = "1 år etter prosedyren") +
...
caption <- paste0(
  "Tabellen viser responsrate for prosedyrene som ble utført i perioden ",
  (periode_data$forste_dato_5aarOppf), " og " ,
  (periode_data$siste_dato_5aarOppf),
  ", og der oppfølging ble besvart henholdsvis i ",
  periode_data$sisteHeleYear,
  " og hittil i ",
  periode_data$nyesteRegYear, "." )
