...
abstract: Denne rapporten oppsummerer nøkkeltall for registreringer i AblaNor utført av  `r params$hospitalName` de siste 12 månedene . Bare forløp som har resultert i en prosedyre (eventuelt en avbrutt prosedyre) teller med i datagrunnlaget. Opptellingene er gjort per forløp i perioden, det vil si at en pasient som har hatt mer enn et forløp vil telle flere ganger.  
...
# Forløpstype
Tabellen under (Tabell \@ref(tab:typeForlopAar)) viser sammenlagt antall forløp
registrert i AblaNor fra `r params$hospitalName` de `r showN` siste 
månedene. Alle registreringer med prosedyredato i intervallet 
`r format(periode_data$forste, "%d/%m-%Y")` til
`r format(periode_data$siste_dato, "%d/%m-%Y")` er inkludert i denne
opptellingen. Figuren (Figur \@ref(fig:typeForlopFig)) viser antall forløp per
måned, samt fordeling av forløpstypene. 
```{r typeForlopAar}
...
```{r typeForlopFig, fig.cap = paste("Fordeling av forløpstyper registrert de ", showN," siste månedene."), fig.pos = "H", fig.align = "center", out.width = "100%" }
...
ggplot2::ggplot(data = d_forlop_mnd, aes(y = n, x = maaned, fill = forlopstype_tekst)) +
  ...
  ggplot2::geom_text(aes(label = n_uten0), position = position_stack(vjust = .5)) +
...
# Komplikasjoner
Tallgrunnlaget for figuren og tabellen under er alle registreringer gjort i 
AblaNor fra `r params$hospitalName` med prosedyredato i intervallet 
`r format(periode_data$forste, "%d/%m-%Y")` til
`r format(periode_data$siste_dato, "%d/%m-%Y")`. 

Figur \@ref(fig:komplikasjonerFig) viser prosentvis fordeling 
av forløp med minst en komplikasjon. Tallet i søylen viser antall forløp med 
komplikasjon delt på totalt antall forløp den måneden. 


Tabell \@ref(tab:komplikasjonerMaanedTabell) viser andel forløp
med komplikasjoner, totalt per måned og i undergruppene: 
Forløpstype, aldersklasse <75 eller >=75 år og BMI-klasse <35 eller >=35.
Merk at dersom Forløpstype, alder eller BMI mangler for et forløp, teller 
ikke dette forløpet i undergruppene.
```{r komplikasjonerFig, fig.cap = "Andel komplikasjoner per måned.", fig.pos = "H", fig.align = "center", out.width = "100%"}
...
cap <- "Andel komplikasjoner per måned, totalt og for undergrupper."
...
Tallgrunnlaget for figuren og tabellen under er registreringer gjort i 
AblaNor fra `r params$hospitalName` med prosedyredato i intervallet 
`r format(periode_data$forste, "%d/%m-%Y")` til
`r format(periode_data$siste_dato, "%d/%m-%Y")`. Elektrofysiologiske
undersøkelse eller forløp der akutt suksess er lik "Ikke aktuelt" er fjernet fra 
datagrunnlaget. 

Andel forløp med akutt suksess er regnet ut ved å dele antall
forløp der akutt suksess er lik "Ja" på totalt antall forløp der akutt suksess 
er en av "Ja", "Nei" eller "Usikker". 

```{r akuttSuksessFig, fig.cap = paste0("Andel forløp med akutt suksess per måned de ", showN, " siste månedene. I datagrunnlaget telles kun abladerte pasienter.") , fig.pos = "H", fig.align = "center", out.width = "100%"}
...
cap <- paste0("Andel forløp med akutt suksess. ", 
              ...
              "Opptellingen er gjort per måned de ", showN, " siste månedene. ",  
              "Vi ser andel akutt suksess blandt alle ablanderte (Totalt), ", 
              "per forløpstype, ", 
              "samt for undergrupper av AFLI og VT. ", 
              "Merk at grunnet manglende registrering av tilhørighet i ", 
              "undergrupper av AFLI/VT for et fåtall av forløp har vi små uoverstemmelser i nevnerne.")
...
knitr::kable(x = d_akuttSuksess_TOTAL,
  ...
  kableExtra::add_header_above(c(" " = 1,
                     ...
                     "VT, Kardiomyopati" = 5,
                     " " = 1)) %>%
  ablanor::mst_short(., type = params$tableFormat, lsd = TRUE, fs = 9,
            full_width = FALSE)
```
<br>
...
```{r figProsedyreVarighetMin, fig.cap = "Prosedyrevarighet [min] per måned.", fig.align = 'center',  out.width = "100%"}
...
```{r figRontgenVarighetMin, fig.cap = "Røntgentid [min] per måned.", fig.align = 'center',  out.width = "100%"}
