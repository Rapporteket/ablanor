...
d_hendelse_nasjonalt <- ablanor::getHendelse(
  singleRow = FALSE, 
  reshId = params$reshId,
  userRole = "SC", 
  fromDate = NULL, 
  toDate = NULL)$d_hendelse %>% 
...
d_basereg_nasjonalt <- ablanor::getBasereg(
  singleRow = FALSE, 
  reshId = params$reshId,
  userRole = "SC", 
  fromDate = periode_data$forste_dato, 
  toDate = periode_data$siste_dato)$d_basereg %>% 
  dplyr::select(MCEID, CENTREID, HOYDE, VEKT)


d_pros_nasjonalt <- ablanor::getPros(
  singleRow = FALSE, 
  reshId = params$reshId,
  userRole = "SC", 
  fromDate = periode_data$forste_dato, 
  toDate = periode_data$siste_dato)$d_pros %>% 
  dplyr::select(MCEID, CENTREID, FORLOPSTYPE, DATO_PROS)


d_mce_nasjonalt <- ablanor::getMce(
  singleRow = FALSE, 
  reshId = params$reshId,
  userRole = "SC", 
  fromDate = periode_data$forste_dato, 
  toDate = periode_data$siste_dato)$d_mce %>% 
...
```{r pagebreak_kl_eff_1aar, results = "asis", eval = (params$tableFormat == "pdf")}
cat("\n\n\\pagebreak\n")
```


# Klinisk effekt 1 år etter prosedyren
...
```{r pagebreak_kl_eff_5aar, results = "asis", eval = (params$tableFormat == "pdf")}
cat("\n\n\\pagebreak\n")
```
