...
(NB foreløpig er dette en test-versjon av rapportetket, på datadump(csv fil) fra januar 2021).
<br>
<br>
 `r  if(params$registryName !="test_ablanor_lokalt")  {"-->"}`
...
Denne rapporten inneholder alle forløp registrert av  `r params$author` de siste
...
Denne rapporten inneholder alle forløp registret ved `r params$hospitalName` de 
...
```{r pagebreak_1, results = "asis", eval = (params$tableFormat == "latex")}
  cat("\n\n\\pagebreak\n")
```
...
d_forlop_mnd <- d_ablanor %>%
  count(maaned, forlopstype, .drop = FALSE) %>%
...
```{r pagebreak_2, results = "asis", eval = (params$tableFormat == "latex")}
  cat("\n\n\\pagebreak\n")
```
...
```{r pagebreak_3, results = "asis", eval = (params$tableFormat == "latex")}
  cat("\n\n\\pagebreak\n")
```
...
d_akutt_suksess_AFLI_icd = d_ablanor %>%
  ...
  ablanor::ki_akutt_suksess() %>%
  dplyr::group_by(kategori_afli_aryt_i48, maaned) %>%
  ablanor::aggreger_ki_prop() %>%
  dplyr::transmute(maaned = maaned,
...
d_akutt_suksess_VT<- d_ablanor %>%
  ...
  ablanor::ki_akutt_suksess() %>%
  dplyr::group_by(kategori_vt_kardiomyopati, maaned) %>%
  ablanor::aggreger_ki_prop() %>%
  dplyr::transmute(maaned = maaned,
...
d_komplikasjonar_pacemaker2 <- left_join(
  d_komplikasjonar_pacemaker %>% tidyr::expand(., maaned), 
  d_komplikasjonar_pacemaker,
  by = "maaned") %>% 
  tidyr::replace_na(., list(ki_teller = 0, ki_nevner = 0)) %>% 
  arrange(maaned)
...
```{r pagebreak_4, results = "asis", eval = (params$tableFormat == "latex")}
  cat("\n\n\\pagebreak\n")
```
...
```{r pagebreak_5, results = "asis", eval = (params$tableFormat == "latex")}
  cat("\n\n\\pagebreak\n")
```
